...
test('user navigation and form filling', async () => {
  ...
  userEvent.type(favoriteFoodInput, '')
  expect(favoriteFoodInput).toHaveValue('')
  ...
  expect(screen.getByLabelText(/food/i)).toHaveTextContent('')
  ...
  // 28 - Un titre "Oh no. There was an error." est dans le document
  await waitFor(() =>
  ...
  //32 - l'utilisateur clique sur le lien "tryAgain"
